program CurveFit
  ...
  open(unit = out_unit, file = "fit.dat", status = "replace", action = "write", iomsg = errormsg, iostat = istat)      
  if (istat /= 0) print *, errormsg
  N = 0
  do i = 1, 10000000
  ...
  nablaS = 0
  do i = 1, N
    nablaS(1) = nablaS(1) + ((2.0_dp / N) * abs(data(i,2) - fit(i,2)) * (-1) * (exp(a(2) / fit(i,1))))
    nablaS(2) = nablaS(2) + ((2.0_dp / N) * abs(data(i,2) - fit(i,2)) * (-1) * (a(1) / fit(i,1)) * (exp(a(2) / fit(i,1))))
  end do
